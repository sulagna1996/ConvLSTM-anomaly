...
class MeltpoolLightning(pl.LightningModule):

    def __init__(self, hparams=None, model=None):
        super(MeltpoolLightning, self).__init__()

        # default config
        self.path = 'MP_Train2\\'#os.getcwd() + '/data'
...
def run_trainer():
    ...
    model = MeltpoolLightning(model=conv_lstm_model)
    ...
    trainer.fit(model)
    trainer.save(model.state_dict('trained_params.pt'))
