...
if __name__ == "__main__":
    ...
    print('ConvLSTM Laser Hot Wire AM Anomaly Detection Algorithm Version 2.1')
    ...
    print('To view a model code guide type "help".')
    while True:
    ...
    if model_code == "02":
        ...
        if getattr(sys, 'frozen', False):
            ...
            model.load_state_dict(torch.load(os.path.join(sys._MEIPASS, 'FLIR_Sept2.pt'), map_location=device))
        else:
            #model.load_state_dict(torch.load('trained_params_20w_size.pt', map_location=device))
            model.load_state_dict(torch.load('FLIR_Sept2.pt', map_location=device))
        n_steps_past = 20
        n_steps_ahead = 20
        clip_const=39
        while True:
    ...
    k = Parallel_Preprocessing(video_filename, evaluate=True, mp_detect=object_detection)
    num_frames = k.frame_count - 11
